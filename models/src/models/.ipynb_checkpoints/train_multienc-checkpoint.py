from models.forecastors.utils.multi_encengine import *
from models.forecastors.mulit_encoder_unet.muti_encunet import *
from models.forecastors.utils.utils import *
from models.forecastors.utils.loss import * 
from data_prep.dataset.mulitdataset import *
...
parser.add_argument("--validation_batch_size", "-vbs",type = int,default = 3)
parser.add_argument("--train_batch_size", "-tbs",type = int,default = 3)
parser.add_argument("--experiment", "-exp",type = int,default = 0)
parser.add_argument("--loss_type", "-lt",type = str,default = "ALL")
parser.add_argument("--window_size", "-ws",type = int,default = 17)
...
if __name__ =="__main__" :
    ...
    TrainData = WRFDataset(window_size = args.window_size, data_type = "train")
    print("Loading ValidData...")
    ValidData = WRFDataset(window_size = args.window_size, data_type = "valid")
    input_features = 2
    output_feature = 1
    print("Define Model...")
    model = UNet(in_channels = input_features, out_channels = output_feature)
    ...
    if (args.loss_type == "PSNR" ) or (args.loss_type == "SSIM") or (args.loss_type == "MIX"):
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=2e-3,maximize = True)
