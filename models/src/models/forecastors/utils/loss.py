...
class Loss(nn.Module):
    ...
    def forward(self,yhat,y):
        if self.loss_name == "RMSE":
            ...
            return self.ssim(yhat,y)
        elif self.loss_name == "MIX":
            com_loss = self.psnr(yhat,y) + 10 * self.ssim(yhat,y)
            return com_loss
        elif self.loss_name == "ALL":
            com_loss = self.l1(yhat,y) - 10*self.ssim(yhat,y) - 5*self.psnr(yhat,y)
            return com_loss