...
class Training():
    ...
    def train_fn(self,model,train_loader):
        ...
        for index,train_batch in tqt:
            for key in train_batch.keys(): 
                train_batch[key] = train_batch[key].to(self.train_config.device)
            target = train_batch["target"]
            self.train_config.optimizer.zero_grad()
            pred_target = model(train_batch)
    ...
    def valid_fn(self,model,validation_loader):
        ...
        with torch.no_grad():
            for index, valid_batch in tqt :
                for key in valid_batch.keys():
                    valid_batch[key] = valid_batch[key].to(self.train_config.device)
                target = valid_batch["target"].to(self.train_config.device)
                self.train_config.optimizer.zero_grad()
                pred_target = model(valid_batch)
                pred_target = pred_target.squeeze(1)
    ...
    def get_dataloader(self,train_dataset,valid_dataset):
        ...
        valid_sampler = RandomSampler(valid_dataset,replacement = True,num_samples = 200)
    ...
    def fit(self,train_dataset,valid_dataset):
        ...
        self.train_config.model.to(self.train_config.device)
