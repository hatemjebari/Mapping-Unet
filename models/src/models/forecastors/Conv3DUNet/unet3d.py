    ...
    def forward(self, x, down_sampling_features):
        ...
        for k, op in self.module_dict.items():
            if k.startswith("deconv"):
                ...
                print("downscalign ", down_sampling_features[int(k[-1])].shape)
...
class UnetModel(nn.Module):
    ...
    def forward(self, x):
        enc, downsampling_features = self.encoder(x)
        out = self.decoder(enc, downsampling_features)
        return out

if __name__ == "__main__":
    # x has shape of (batch_size, channels, depth, height, width)
    device = torch.device("cuda:1")
    x_test = torch.randn(1, 6, 20, 213, 288)
    ...
    print("The output of unet models is", out.shape)